...
//            $query = query();
    ...
    {
        try {
            $data = Product::where('hide','0')->withCount('order')->orderBy('order_count', 'desc')->with('store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->where('discount < price')->orderBy('discount','desc')->limit(8)->get();
            foreach (  $data  as $item) {
                $sumStars = CmtRating::where('product_id',$item->id)->avg('point');
                 $item->totalRating = floor($sumStars);
                 $totalComment = count(CmtRating::where('product_id',$item->id)->get());

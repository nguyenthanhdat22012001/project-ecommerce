...
//            $query = query();
    ...
    {
        try {
            $data = Product::where('hide','0')->withCount('order')->with('store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->orderBy('discount','DESC')->inRandomOrder()->limit(8)->get();
            foreach (  $data  as $item) {
                $sumStars = CmtRating::where('product_id',$item->id)->avg('point');
                 $item->totalRating = floor($sumStars);
                 $totalComment = count(CmtRating::where('product_id',$item->id)->get());

...
//            $query = query();
        ...
        if($key == 'cate'){
            $data = Product::where('hide','1')->whereRaw("cate_id = ". $id )->with('rating','store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->get();
        }
        if($key == 'brand'){
            $data = Product::where('hide','1')->whereRaw("brand_id = ". $id )->with('rating','store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->get();
        }
        if($key == 'store'){
            $data = Product::where('hide','1')->whereRaw("store_id = ". $id )->with('rating','store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->get();
        }
        // $result = $data->get();
    ...
    {
        try {
            $data = Product::where('hide','1')->withCount('order')->orderBy('order_count', 'desc')->with('store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->orderBy('discount','desc')->limit(9)->get();
            foreach (  $data  as $item) {
                $sumStars = CmtRating::where('product_id',$item->id)->avg('point');
                 $item->totalRating = floor($sumStars);
                 $totalComment = count(CmtRating::where('product_id',$item->id)->get());
    ...
    {
        try {
            $data = Product::where('hide','1')->withCount('order')->orderBy('order_count', 'desc')->with('store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->limit(9)->get();
            foreach (  $data  as $item) {
                $sumStars = CmtRating::where('product_id',$item->id)->avg('point');
                 $item->totalRating = floor($sumStars);
                 $totalComment = count(CmtRating::where('product_id',$item->id)->get());
    ...
    {
        try {
            $data = Product::where('hide','1')->with('rating')->get();
            foreach ( $data  as $item) {
                $sumStars = CmtRating::where('product_id',$item->id)->avg('point');
                $item->totalRating = floor($sumStars);
            }
    ...
    {
        try {
            $data = Store::where('hide','1')->withCount('follow')->orderBy('follow_count', 'desc')->limit(5)->get();
            return response()->json([
                'success' => true,
                'message'=>  'Tìm thành công',
                'data'=>$data
    ...
    {
        try {
          $data=Product::where('hide','1')->with('rating','store:id,name,slug','cate:id,name,slug','brand:id,name,slug')->orderBy('created_at','DESC')->get();
